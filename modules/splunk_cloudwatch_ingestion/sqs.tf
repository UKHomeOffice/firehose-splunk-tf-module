...
resource "aws_sqs_queue" "transform_lambda_dlq" {
  name                      = "${var.environment_prefix_variable}-${var.environment_prefix_variable}-splunk-fh-transform-dlq"
...
resource "aws_sqs_queue" "retry_sqs_dql" {
    name                = "${var.environment_prefix_variable}-${var.environment_prefix_variable}-splunk-fh-retry-dlq"
    kms_master_key_id   = aws_kms_key.firehose_key.id
}